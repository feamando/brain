#!/usr/bin/env python3
"""
PM-OS Brain Index Generator (Standalone)
...
Usage:
    ...
    python3 brain_index_generator.py --config team.yaml # Load team config from YAML
"""
...
# Add pip-package source to path so pmos_brain can be imported if installed
...
class BrainIndexGenerator:
    ...
    def __init__(
        ...
        if brain_path is None:
            brain_path = Path(__file__).parent.parent / "brain"
    ...
    def _load_tier1_entities(self) -> List[Dict[str, Any]]:
        ...
        user_config = team_config.get("user", {})
        tier1 = []
    ...
    def _get_hot_topic_ids(self) -> List[str]:
        """Get entity IDs from today's context via pmos_brain scanning (if available)."""
        try:
            from pmos_brain.loader.brain_loader import (
                build_alias_index,
...
def main():
    ...
    # Load team config from YAML file if provided
    team_config: Optional[Dict[str, Any]] = None
    if args.config:
