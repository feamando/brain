...
class SchemaMigrator:
    ...
    def _migrate_parallel(
        self, files: List[Path], workers: int
    ) -> Dict[str, Any]:
        """Migrate files in parallel."""
        with ThreadPoolExecutor(max_workers=workers) as executor:
            futures = {
                executor.submit(self.migrate_file, f): f for f in files
            }
    ...
    def _extract_relationships(
        ...
        for match in matches:
            ...
            relationships.append(
                Relationship(type="related_to", target=target)
            )
    ...
    def _build_v2_frontmatter(
        ...
        v2 = {
            ...
            "$events": [
                {
                    "event_id": f"evt-migration-{now.strftime('%Y%m%d')}",
        ...
        skip_fields = {
            "name", "tags", "aliases", "status", "created", "updated",
            "source", "manager", "team", "owner"
        }
