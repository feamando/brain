...
def get_config(user_path: Optional[Path] = None, force_reload: bool = False) -> ConfigLoader:
    """
...
def get_jira_config() -> dict:
    ...
    return {
        ...
        'username': config.get("integrations.jira.username") or os.getenv("JIRA_USERNAME"),
...
def get_gemini_config() -> dict:
    ...
    return {
        ...
        'model': config.get("integrations.gemini.model") or os.getenv("GEMINI_MODEL", "gemini-2.5-flash")
    }
...
def get_github_config() -> dict:
    ...
    return {
        ...
        'org': config.get("integrations.github.org") or os.getenv("GITHUB_ORG", "hellofresh")
    }
...
def get_slack_config() -> dict:
    ...
    return {
        ...
        'channel': config.get("integrations.slack.channel") or os.getenv("SLACK_CHANNEL", "C0A6ZAS1MSQ")
    }
...
def get_slack_mention_bot_name() -> str:
    ...
    Returns:
        Bot name (e.g., 'ngo-slack-connect') without the @ prefix.
...
def get_meeting_prep_config() -> dict:
    ...
    return {
        ...
        'task_inference': config.get("meeting_prep.task_inference", default_task_inference),
        'section_defaults': config.get("meeting_prep.section_defaults", default_section_defaults),
        'type_overrides': config.get("meeting_prep.type_overrides", default_type_overrides),
    }
...
def get_product_ids() -> List[str]:
    ...
    Returns:
        List of product ID strings (e.g., ['good-chop', 'the-pets-table'])
    """
...
def get_product_by_id(product_id: str) -> Optional[dict]:
    ...
    Args:
        product_id: Product identifier (e.g., 'good-chop')
...
def get_product_by_jira_project(jira_project: str) -> Optional[dict]:
    ...
    Args:
        jira_project: Jira project key (e.g., 'GOC')
...
def get_report_by_id(report_id: str) -> Optional[dict]:
    ...
    Args:
        report_id: Report identifier (e.g., 'beatrice-dimarucut')
...
def get_workspace_config() -> dict:
    ...
    return config.get("workspace", {
        ...
        'standard_subfolders': ['discovery', 'planning', 'execution', 'reporting', 'presentations', 'discussions'],
        'context_sync': {'enabled': True, 'sync_on_boot': True, 'bidirectional': True}
    })
...
def get_standard_subfolders() -> List[str]:
    ...
    return workspace.get('standard_subfolders', [
        'discovery', 'planning', 'execution', 'reporting', 'presentations', 'discussions'
    ])
