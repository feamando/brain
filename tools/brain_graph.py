...
class BrainGraph:
    ...
    def expand(self, seeds: List[SearchResult],
                ...
                for rel in relationships:
                    ...
                    if canonical_target not in neighbors or score > neighbors[canonical_target].score:
                        neighbors[canonical_target] = SearchResult(
                            ...
                            score=score,
                            source='graph',
                            match_reasons=[f"via {via_entity} ({rel.get('type', 'related_to')})"],
                            via=via_entity,
    ...
    def _load_entity(self, entity_id: str) -> Optional[Dict]:
        ...
        file_path = self.resolver.get_entity_path(canonical_id) if hasattr(self.resolver, 'get_entity_path') else None
    ...
    def _find_entity_file(self, entity_id: str) -> Optional[Path]:
        ...
        # project/growth-initiative -> Projects/Growth_Initiative.md
        ...
        if entity_type == 'entity' and subtype:
            ...
            subdir = subtype.capitalize() + 's' if not subtype.endswith('s') else subtype.capitalize()
        ...
        for pattern in [f"**/{filename}", f"**/{name}.md", f"**/{name.replace('-', '_')}.md"]:
