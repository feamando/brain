...
class GDocsEnricher(BaseEnricher):
    ...
    def _classify_document(self, title: str, content: str) -> str:
        ...
        if any(kw in title_lower for kw in ["prd", "product requirement", "spec"]):
            ...
            return "rfc"
        elif any(kw in title_lower for kw in ["adr", "decision record", "architecture"]):
            return "adr"
    ...
    def _calculate_completeness(self, frontmatter: Dict[str, Any]) -> float:
        ...
        optional_fields = [
            "$relationships", "$tags", "$aliases",
            "role", "team", "owner", "description"
        ]
        ...
        required_score = present_required / len(required_fields) if required_fields else 1
        optional_score = present_optional / len(optional_fields) if optional_fields else 0
    ...
    def enrich_orphans(
        ...
        target_types = entity_types or ["project", "person", "system", "team", "squad", "experiment"]
        ...
        for orphan_path, frontmatter, body in orphans:
            ...
            gdocs_matches = self._search_gdrive(
                drive_service, entity_name, aliases
            )
            results["api_calls"] += 1
