...
class GitHubEnricher(BaseEnricher):
    ...
    def enrich_orphans(
        ...
        target_repos = repos or ["example-org/web"]
        ...
        for orphan_path, frontmatter, body in orphans:
            ...
            github_matches = self._search_github(
                entity_name, aliases, target_repos
            )
            results["api_calls"] += 1
            ...
            for repo in repos:
                try:
                    ...
                    result = subprocess.run(
                        [gh_path, "api", f"search/issues?q=repo:{repo}+is:pr+{term}+in:title&per_page=5"],
                        capture_output=True,
                        text=True,
                        timeout=15,
                    )
