...
INDEX_DIRS = ["Entities", "Projects", "Experiments", "Architecture", "Decisions", "Strategy", "Technical"]
...
class BrainIndex:
    ...
    def stats(self) -> Dict[str, Any]:
        ...
        token_counts = [(token, len(entities)) for token, entities in self.index.items()]
        ...
        return {
            ...
            "avg_postings_per_token": sum(len(v) for v in self.index.values()) / len(self.index) if self.index else 0
        }
