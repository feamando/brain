...
def create_entity_file(name: str, entity_type: str, context: str, source: str, use_v2: bool = True) -> str:
    """
...
def _create_v2_entity_file(name: str, entity_type: str, context: str, source: str) -> str:
    ...
    frontmatter = f"""---
$schema: brain://entity/{entity_type}/v1
$id: entity/{entity_type}/{slug}
$type: {entity_type}
$version: 1
...
$events:
  - event_id: evt-{now.strftime('%Y%m%d')}-001
    ...
    changes:
      - field: $schema
        operation: set
        value: brain://entity/{entity_type}/v1
    message: Created from {source} analysis
name: {name}
...
def _create_v1_entity_file(name: str, entity_type: str, context: str, source: str) -> str:
...
def write_decision(decision, source: str, source_context: str, dry_run: bool = False) -> Optional[Path]:
    ...
    context = decision.get("context", decision.get("rationale", "[No context provided]"))
...
def process_gdocs_analysis(analysis: dict, aliases: dict, state: dict, dry_run: bool) -> dict:
    ...
    for result in results:
        ...
        for entity in result.get("entities", []):
            ...
            if name and len(name) > 2:
                ...
                if update_entity_context(name, etype, context_entry, "gdocs", aliases, state, dry_run):
                    stats["entities"] += 1

    return stats


def process_slack_analysis(analysis: dict, aliases: dict, state: dict, dry_run: bool) -> dict:
    ...
    for entity in analysis.get("entities", []):
        ...
        if name and len(name) > 2:
            context_entry = f"- [{datetime.now().strftime('%Y-%m-%d')}] {context} (#{channel_name})"
            if update_entity_context(name, etype, context_entry, "slack", aliases, state, dry_run):
                stats["entities"] += 1

    return stats


def process_github_analysis(analysis: dict, aliases: dict, state: dict, dry_run: bool) -> dict:
    ...
    for entity in analysis.get("entities", []):
        ...
        if name and len(name) > 2:
            context_entry = f"- [{datetime.now().strftime('%Y-%m-%d')}] {context} (GitHub commits)"
            if update_entity_context(name, etype, context_entry, "github", aliases, state, dry_run):
                stats["entities"] += 1
...
def process_analysis_file(filepath: Path, source: str, aliases: dict, state: dict, dry_run: bool) -> dict:
...
def run_brain_writer(source: str, dry_run: bool = False, resume: bool = True):
    ...
    for filepath in analysis_files:
        ...
        if not dry_run:
            state.setdefault("sources_processed", {}).setdefault(processed_key, []).append(str(filepath))
            state["decisions_logged"] = state.get("decisions_logged", 0) + stats.get("decisions", 0)
            state["total_context_added"] = state.get("total_context_added", 0) + stats.get("entities", 0)
            save_state(state)
