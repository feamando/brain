...
try:
    ...
    from embedding_edge_inferrer import EmbeddingEdgeInferrer, EdgeInferenceReport
...
class BrainEnrichmentOrchestrator:
    ...
    SOFT_EDGE_CONFIG = {
        ...
        "project": {"threshold": 0.92, "limit": 100},  # Higher threshold for projects (many artifacts)
    }
    ...
    def run(
        ...
        entity_types = self.BOOT_ENTITY_TYPES if mode == "boot" else list(self.SOFT_EDGE_CONFIG.keys())

        for entity_type in entity_types:
            config = self.SOFT_EDGE_CONFIG.get(entity_type, {"threshold": 0.85, "limit": 50})
    ...
    def _run_orphan_cleanup(
        ...
        try:
            ...
            if self.verbose:
                print(f"  Found {len(body_report.relationships)} potential relationships")
        ...
        try:
            ...
            result.relationships_created = result.body_relationships_created + gdocs_results.get("relationships_created", 0)

            if self.verbose:
                print(f"  GDocs: enriched {gdocs_enriched} entities, {gdocs_results.get('relationships_created', 0)} relationships")
        except Exception as e:
            if self.verbose:
...
def main():
    ...
    if args.output == "json":
        ...
        print("Improvements:")
        density_pct = (result.density_improvement / max(result.baseline_density, 0.001)) * 100
        print(f"  Density: {result.baseline_density:.3f} -> {result.final_density:.3f} ({density_pct:+.1f}%)")
