...
def extract_key_updates(context_content: str, entity_id: str) -> List[str]:
    ...
    relevant_sections = ['key decisions', 'blockers', 'action items', 'updates', 'status']
...
def _append_v2_event(
    ...
    for ev in existing_events:
        if ev.get("correlation_id") == new_event["correlation_id"] and ev.get("message") == message:
            return False  # Already logged

    # Append event
    if "$events" not in frontmatter:
        frontmatter["$events"] = []
    frontmatter["$events"].append(new_event)
...
def append_changelog_entry(file_path: str, date: str, message: str, dry_run: bool = False) -> bool:
    ...
    # V1 format: use changelog section
...
def main():
    ...
    context_date = date_match.group(1) if date_match else datetime.now().strftime('%Y-%m-%d')
    ...
    for entity_id, data in sorted(significant_matches.items(), key=lambda x: x[1]['count'], reverse=True):
        file_path = data['file']
        full_path = os.path.join(BRAIN_DIR, file_path)
