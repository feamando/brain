...
Examples:
  ...
  - entity/project/growth-initiative
...
class CanonicalResolver:
    ...
    - Path: Projects/Growth_Initiative.md
    - $id: entity/project/growth-initiative
    - Alias: Growth Initiative, GI
    """
    ...
    def __init__(self, brain_path: Path):
        ...
        """
        self.brain_path = Path(brain_path) if not isinstance(brain_path, Path) else brain_path
    ...
    def _load_cache(self) -> bool:
        ...
        try:
            ...
            self._reverse_index = {k: set(v) for k, v in data.get("reverse_index", {}).items()}
            self._entity_paths = {k: Path(v) for k, v in data.get("entity_paths", {}).items()}
            self._built = True
    ...
    def find_similar(self, reference: str, max_results: int = 5) -> List[Tuple[str, float]]:
        """
    ...
    def get_stats(self) -> Dict[str, int]:
        ...
        return {
            ...
            "avg_refs_per_entity": (
                len(self._index) // len(self._entity_paths)
                if self._entity_paths else 0
            ),
        }
