...
""")
    ...
    def test_decay_factor(self, temp_brain_with_relationships):
        ...
        b_result = next((n for n in neighbors if n.entity_id == "entity/test/entity-b"), None)
        assert b_result is not None
        assert b_result.score == 0.5  # 1.0 * 0.5

        # Custom strength should apply to entity-c
        c_result = next((n for n in neighbors if n.entity_id == "entity/test/entity-c"), None)
...
class TestBrainQuery:
    ...
    def test_brain_only_query(self):
        ...
        if not Path("/Users/nikita.gorshkov/pm-os/user/brain").exists():
    ...
    def test_brain_graph_query(self):
        ...
        bq = BrainQuery()
        result = bq.query("factor form", limit=10, use_graph=True)
    ...
    def test_merge_max_score_wins(self):
        ...
        neighbors = [
            SearchResult(entity_id="entity-a", score=0.3, source="graph"),  # Lower score
...
class TestQueryDependence:
    ...
    def test_different_queries_different_results(self):
        ...
        result2 = bq.query("factor form canada", limit=10, use_graph=False)
...
class TestPerformance:
    ...
    def test_search_latency(self):
        ...
        # Measure
        start = time.time()
        result = bq.query("factor form", limit=10, use_graph=False)
