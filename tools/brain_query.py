...
Usage:
    ...
    python brain_query.py "growth initiative" --format json --limit 5
"""
...
class BrainQuery:
    ...
    def _merge_and_rank(self, seeds: List[SearchResult],
                        neighbors: List[SearchResult]) -> List[SearchResult]:
        """
...
def format_text_output(result: QueryResult) -> str:
    ...
    for r in result.results:
        source_icon = {"alias": "A", "content": "C", "graph": "G", "brain+graph": "B+G"}.get(r.source, "?")
...
def format_markdown_output(result: QueryResult) -> str:
    ...
    lines.append(f"## Query: {result.query}")
    lines.append("")
    lines.append(f"**Results:** {len(result.results)} | **Seeds:** {result.seed_count} | **Latency:** {result.latency_ms:.1f}ms")
    lines.append("")
...
Examples:
  ...
  python brain_query.py "growth initiative" --no-graph
  ...
  python brain_query.py "jane" --limit 5 --decay 0.3
    ...
    parser.add_argument("--no-graph", action="store_true", help="Disable graph expansion (BRAIN only)")
    parser.add_argument("--decay", "-d", type=float, default=0.5, help="Graph decay factor (default: 0.5)")
