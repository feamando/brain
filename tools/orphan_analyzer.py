...
def main():
    ...
    parser.add_argument(
        "action",
        choices=["scan", "mark-pending", "mark-standalone", "clear-connected", "report"],
        nargs="?",
        default="scan",
    ...
    if args.action == "scan" or args.action == "report":
        ...
        if args.output == "json":
            ...
            print(f"Orphan rate: {analysis.total_orphans / max(1, analysis.total_entities) * 100:.1f}%")
            print()

            if analysis.orphans_by_type:
                print("Orphans by type:")
                for etype, count in sorted(analysis.orphans_by_type.items(), key=lambda x: -x[1]):
                    print(f"  {etype}: {count}")
                print()

            if analysis.orphans_by_reason:
                print("Orphans by reason:")
                for reason, count in sorted(analysis.orphans_by_reason.items(), key=lambda x: -x[1]):
                    print(f"  {reason}: {count}")
                print()

            if args.action == "report" and analysis.orphan_details:
                print(f"Sample orphans (first {min(20, len(analysis.orphan_details))}):")
