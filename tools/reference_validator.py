...
class ReferenceValidator:
    ...
    # Valid entity types
    VALID_TYPES = {
        "person", "project", "team", "squad", "domain",
        "system", "brand", "experiment", "entity", "reasoning"
    }
    ...
    def validate_entity(
        ...
        for i, rel in enumerate(relationships):
            if isinstance(rel, dict):
                rel_errors, rel_warnings = self._validate_relationship(
                    rel, i, strict
                )
    ...
    def validate_and_normalize(
        ...
        for i, rel in enumerate(relationships):
            if isinstance(rel, dict):
                normalized_rel, rel_errors, rel_warnings = self._normalize_relationship(rel, i)
    ...
    def validate_reference(self, reference: str) -> Tuple[bool, Optional[str], Optional[str]]:
        """
        ...
        if self.CANONICAL_PATTERN.match(reference):
            ...
            if self.resolver.resolve(reference):
                ...
                return (False, None, f"Canonical reference '{reference}' does not exist")
        ...
        if similar:
            ...
            return (False, suggestion, f"Could not resolve '{reference}'. Did you mean '{suggestion}'?")
...
def validate_file(file_path: Path, validator: ReferenceValidator, strict: bool = False) -> ValidationResult:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Validate Brain entity references"
    )
    parser.add_argument(
        "action",
