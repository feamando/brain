...
class EmbeddingEdgeInferrer:
        ...
        for i in range(len(entity_ids)):
            for j in range(i + 1, len(entity_ids)):
                ...
                if sim >= self.threshold:
                    ...
                    edges_by_type_pair[type_pair] = edges_by_type_pair.get(type_pair, 0) + 1
    ...
    def _format_content(
        ...
        yaml_str = yaml.dump(fm_copy, default_flow_style=False, allow_unicode=True, sort_keys=False)
        return f"---\n{yaml_str}---{body}"
...
def main():
    ...
    if not EMBEDDINGS_AVAILABLE:
        print("Warning: sentence-transformers not installed. Using fallback similarity.", file=sys.stderr)
    ...
    if args.action == "scan":
        ...
        if args.output == "json":
            ...
            if report.edges_by_type_pair:
                ...
                for pair, count in sorted(report.edges_by_type_pair.items(), key=lambda x: -x[1]):
                    print(f"  {pair}: {count}")
                print()
        ...
        if args.dry_run:
            ...
            for edge in report.edges[:10]:
                print(f"  {edge.source_id} --[similar_to]--> {edge.target_id} ({edge.similarity:.3f})")
            return 0
