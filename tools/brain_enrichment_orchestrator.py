...
def main():
    ...
    if args.status:
        ...
        if args.output == "json":
            ...
            print(f"State file: {'exists' if status['state_file_exists'] else 'not found'}")
            if status["last_run"]:
    ...
    if args.output == "json":
        ...
        print("Graph Health:")
        print(f"  Orphan rate: {result.orphan_rate_before}% -> {result.orphan_rate_after}%")
        ...
        if result.orphan_rate_after < 30:
            ...
            print(f"Target not met: {result.orphan_rate_after}% >= 30% (run with --mode full)")
